...
function settings(){
 $('view').innerHTML=`<div class="title"><div><p class="eyebrow">БЕЗОПАСНОСТЬ</p><h1>Твой аккаунт<span class="dot">.</span></h1><p class="muted">${profile.must_change_password?'Для продолжения замени временный пароль.':'Здесь можно изменить пароль.'}</p></div></div><form id="password" class="panel narrow"><label>Текущий пароль<input name="currentPassword" type="password" autocomplete="current-password" required></label><label>Новый пароль<input name="password" type="password" minlength="12" maxlength="128" autocomplete="new-password" required></label><label>Повтори новый пароль<input name="repeat" type="password" minlength="12" maxlength="128" autocomplete="new-password" required></label><small>От 12 до 128 символов.</small><button class="primary">Изменить пароль</button></form>`;
