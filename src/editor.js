...
export function attachEditor(host,{value='',onChange=()=>{},onLink=()=>{},onError=()=>{},id='source',imageStore=null}={}) {
 ...
 function keydown(e){
  ...
  if(e.key==='Enter'&&start===end){const m=line.match(/^(\s*)([-*]|\d+\.)(\s+)(\[[ xX]\]\s+)?(.*)$/);if(m){e.preventDefault();if(!m[5].trim())input.setRangeText('',start-line.length,end,'end');else input.setRangeText(`\n${m[1]}${/\d/.test(m[2])?`${parseInt(m[2])+1}.`:m[2]} ${m[4]?'[ ] ':''}`,start,end,'end');sync();return;}if(mode==='live'&&!input.value.includes('```')){e.preventDefault();const i=Number(input.dataset.block),before=input.value.slice(0,start),after=input.value.slice(end);undoStack.push(lastValue);redoStack=[];parts[i]=`${before}\n\n`;parts.splice(i+1,0,after);text.value=parts.join('');lastValue=text.value;emit();liveBlocks();const next=live.querySelectorAll('.live-block')[i+1];next?.click();next?.querySelector('textarea')?.setSelectionRange(0,0);}}
 }
 function liveBlocks(){
  ...
  const formats=[['h1','Заголовок 1','# ',''],['h2','Заголовок 2','## ',''],['h3','Заголовок 3','### ',''],['bold','Жирный · Ctrl/⌘ B','**','**'],['italic','Курсив · Ctrl/⌘ I','*','*'],['strike','Зачёркнутый','~~','~~'],['list','Список','- ',''],['checklist','Подзадачи / чеклист','- [ ] ',''],['link','Ссылка · Ctrl/⌘ K','[','](https://example.com)'],['code','Код','`','`'],['codeBlock','Блок кода','```\n','\n```'],['quote','Цитата','> ',''],['image','Изображение по HTTPS-ссылке','![описание|640](',')','https://example.com/image.jpg']];
