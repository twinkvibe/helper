...
export function attachEditor(host,{value='',onChange=()=>{},onLink=()=>{},onError=()=>{},id='source',imageStore=null,singleBlock=false}={}) {
 ...
 let renderVersion=0;
 const renderInto=(target,value)=>{const version=++renderVersion;const source=imageStore?value.replace(/\((attachment:\/\/[a-zA-Z0-9_-]+)\)/g,(match,url)=>{const data=imageStore.get(url.slice(13));return data?`(${data})`:match;}):value;target.innerHTML=renderMarkdown(source);target.querySelectorAll('pre > code[class*="language-"]').forEach(code=>{const language=code.className.match(/(?:^|\s)language-([\w+-]+)/)?.[1];if(!language||code.parentElement.querySelector('.code-language'))return;const label=document.createElement('span');label.className='code-language';label.textContent=language;code.parentElement.prepend(label);});enhanceDiagrams(target).catch(()=>{}).then(()=>{if(version!==renderVersion)return;});};
