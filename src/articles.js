...
export async function renderPublicArticle(host, client, slug) {
 ...
 host.innerHTML = '<main class="public-article"><header class="public-header"><a class="brand" href="./">h<span>elper</span><i>✳</i></a><a class="quiet" href="./">Войти</a></header><div class="article-cover article-cover-empty"></div><article class="article-paper"><header class="article-title"><h1></h1><p class="eyebrow"></p><div class="article-byline"></div></header><div class="article-body"></div></article></main>';
...
export function mountArticles(host, { client, userId, username, notice, requireSession }) {
 ...
 const render = () => {
  ...
  function edit(article) {
   ...
   form.onsubmit = async event => { event.preventDefault(); const button=form.querySelector('[type=submit]'); button.disabled=true; const status=form.querySelector('.notice'); status.className='notice'; status.textContent='Сохраняем…'; try { article.title=form.elements.title.value; article.slug=slugify(form.elements.slug.value); article.cover_url=form.elements.cover.value; article.published=form.elements.published.checked; article.body=editor.getValue(); await save(article); notice(article.published ? 'Статья опубликована.' : 'Черновик сохранён.'); render(); } catch (error) { status.textContent=error?.message || 'Не удалось сохранить статью.'; status.classList.add('error'); } finally { button.disabled=false; } };
   pane.append(form);
  }
 };
