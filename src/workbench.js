...
export function mountWorkbench(host,{client,userId,initial='todos',notice,requireSession=async()=>true}) {
 ...
 function render(){
  ...
  const top=el('div','work-heading');top.append(el('h1','',section==='todos'?'Задачи':'Заметки'));host.append(top);
 ...
 function renderTasks(){
  ...
  const searchWrap=el('label','search-field');searchWrap.append(icon('search'));const query=el('input');query.id='task-search';query.placeholder='Поиск или #тег';query.setAttribute('aria-label','Поиск задач и тегов');query.value=filters.query;query.oninput=()=>{filters.query=query.value;filters.tag='';renderTasks();};searchWrap.append(query);
  ...
  const todayButton=button(filters.todayOnly?'Сегодня ✓':'Сегодня',()=>{filters.todayOnly=!filters.todayOnly;renderTasks();},filters.todayOnly?'filter-chip active':'filter-chip');todayButton.prepend(icon('today'));
  tools.append(searchWrap,list,todayButton,iconButton('refresh','Обновить',()=>run(refresh)));area.append(tools);
  const allTags=[...new Set(tasks.flatMap(task=>[...(task.tags||[]),...tagsIn(task.description)]))].sort();
  if(allTags.length){const tags=el('div','tag-filter');tags.append(el('span','muted','Теги'));allTags.forEach(value=>tags.append(button(`#${value}`,()=>{filters.query=filters.query.replace(/(?:^|\s)#[\p{L}\p{N}_-]{1,40}/gu,'').trim();filters.tag=filters.tag===value?'':value;renderTasks();},filters.tag===value?'tag active':'tag')));area.append(tags);}
 ...
 function renderNotes(){
  ...
  const actions=el('div','note-actions');const create=button('Новая заметка',()=>{trash=false;newNote();renderNotes();},'primary');create.prepend(icon('plus'));const menu=el('details','action-menu');const summary=el('summary','icon-button');summary.title='Действия с заметками';summary.setAttribute('aria-label','Действия с заметками');summary.append(icon('more'));const menuBody=el('div','action-menu-body');const importButton=button('Импортировать',()=>importFile.click());importButton.prepend(icon('upload'));const exportButton=button('Экспортировать все',exportAll);exportButton.prepend(icon('download'));const trashButton=button(trash?'Вернуться к заметкам':'Открыть корзину',()=>{trash=!trash;renderNotes();});trashButton.prepend(icon('trash'));menuBody.append(importButton,exportButton,trashButton);menu.append(summary,menuBody);actions.append(create,menu);
  ...
  const noteSearchWrap=el('label','search-field note-search');noteSearchWrap.append(icon('search'));const search=el('input');search.placeholder='Поиск заметок или #тег';search.setAttribute('aria-label','Поиск заметок');search.value=noteSearch;noteSearchWrap.append(search);area.append(noteSearchWrap);const noteTags=el('div','tag-filter compact note-tag-filter');const availableTags=[...new Set(activeNotes().flatMap(note=>tagsIn(note.body)))].sort();availableTags.forEach(value=>noteTags.append(button(`#${value}`,()=>{noteSearch=noteSearch===`#${value}`?'':`#${value}`;renderNotes();},noteSearch===`#${value}`?'tag active':'tag')));if(availableTags.length)area.append(noteTags);
  const layout=el('div','notes-layout'),side=el('div','note-sidebar'),main=el('div','note-main');layout.append(side,main);area.append(layout);const links=el('div','note-list');side.append(links);
  ...
  const tools=el('div','note-actions document-actions');const downloadButton=iconButton('download','Скачать Markdown',()=>download((note.title||'Заметка').replace(/[/\\]/g,'_')+'.md',note.body));const linkButton=iconButton('link','Скопировать ссылку на заметку',()=>{const link=new URL(location.href);link.hash=`note=${note.id}`;navigator.clipboard?.writeText(link.href).then(()=>notice('Ссылка скопирована. Она открывает заметку только в браузере с её локальными данными.')).catch(()=>notice(link.href));});const deleteButton=iconButton('trash','Переместить в корзину',()=>{note.deleted=true;save();renderNotes();});tools.append(downloadButton,linkButton,deleteButton);main.append(tools);
